...
#                 'daily_auto_refresh': '5:00 PM'
#             },
#             'note': f'Analysis complete. Successfully processed {len(results)} out of {total_stocks} assets.',
#             **results
#         }
...
# UPDATED: Initialize OpenRouter client instead of Claude
openrouter_client = None
if OPENROUTER_API_KEY != "sk-or-v1-your-openrouter-api-key-here":
    openrouter_client = "configured"  # We'll use requests for OpenRouter
...
# ================= UPDATED STOCK CONFIGURATION =================
def get_filtered_stocks():
    """UPDATED: Get expanded list of popular stocks - 30 US + 15 Nigerian + 20 Crypto = 65 total"""
    # UPDATED: Expanded from 10 to 30 US stocks
    us_stocks = [
    ...
    # UPDATED: Expanded from 10 to 20 crypto stocks
    crypto_stocks = [
    ...
    return {
        'twelve_data_us': us_stocks,
        'naijastocks_nigerian': nigerian_stocks,
        'twelve_data_crypto': crypto_stocks,  # UPDATED: Try TwelveData for crypto first
        'cryptcompare_crypto_fallback': crypto_stocks,  # Keep as fallback
        'us_stocks': us_stocks,
        'nigerian_stocks': nigerian_stocks,
        'crypto_stocks': crypto_stocks,
        'total_count': len(us_stocks) + len(nigerian_stocks) + len(crypto_stocks)
...
def fetch_crypto_data_twelve_data(symbol, interval="1day", outputsize=100, max_retries=TWELVE_DATA_RETRY_ATTEMPTS):
    """UPDATED: Fetch crypto data from TwelveData (new function)"""
    for attempt in range(max_retries):
        try:
            wait_for_rate_limit_twelve_data()
            
            # TwelveData crypto symbols format
...
def fetch_stock_data(symbol, interval="1day", outputsize=100, source="twelve_data"):
    """UPDATED: Unified function to fetch data from multiple sources with crypto fallback"""
    if source == "twelve_data":
        return fetch_stock_data_twelve_with_retry(symbol, interval, outputsize)
    elif source == "twelve_data_crypto":
        # UPDATED: Try TwelveData for crypto first
...
# ================= HIERARCHICAL ANALYSIS SYSTEM =================
def analyze_stock_hierarchical(symbol, data_source="twelve_data"):
    """UPDATED: Analyze stock with hierarchical timeframe dependency and better error handling"""
    try:
        logger.info(f"Starting hierarchical analysis for {symbol} using {data_source}")
        ...
        # UPDATED: Handle crypto with TwelveData first, then CryptoCompare fallback
                ...
                if tf_name == 'daily':
                    ...
                    try:
                        if tf_name == 'monthly':
                            ...
                            # Try to fetch 4h data directly from TwelveData
        ...
        for tf_name, data in timeframe_data.items():
            ...
            # UPDATED: Better handling of empty timeframe data
            if data.empty:
                analyses[f"{tf_name.upper()}_TIMEFRAME"] = {
                    'status': 'Not Available',
            ...
            if analysis:
                ...
                # UPDATED: Provide fallback structure even when analysis fails
                analyses[f"{tf_name.upper()}_TIMEFRAME"] = {
...
# ================= UPDATED OPENROUTER AI INTEGRATION =================
def generate_ai_analysis(symbol, stock_data):
    """UPDATED: Generate detailed AI analysis using OpenRouter instead of Claude"""
    ...
    try:
        ...
        # UPDATED: Use OpenRouter API instead of Claude
        headers = {
...
# ================= UPDATED OPTIMIZED ANALYSIS FUNCTION =================
def analyze_all_stocks_optimized():
    """UPDATED: Optimized stock analysis with expanded stock counts and crypto fallback"""
    try:
        reset_progress()
        
        stock_config = get_filtered_stocks()
        twelve_data_us = stock_config['twelve_data_us']  # Now 30 stocks
        naijastocks_nigerian = stock_config['naijastocks_nigerian']  # Still 15 stocks
        twelve_data_crypto = stock_config['twelve_data_crypto']  # Now 20 crypto stocks
        ...
        # Process Nigerian stocks (unchanged)
        if naijastocks_nigerian:
            batch_size = NAIJASTOCKS_BATCH_SIZE
            num_batches = math.ceil(len(naijastocks_nigerian) / batch_size)
        ...
        # UPDATED: Process Crypto assets (now 20) with TwelveData first, CryptoCompare fallback
        if twelve_data_crypto:
            ...
            for batch_idx in range(num_batches):
                ...
                for symbol in batch_symbols:
                    try:
                        update_progress(processed_count, total_stocks, symbol, f'Analyzing crypto: {symbol}')
                        # UPDATED: Use twelve_data_crypto which tries TwelveData first, then CryptoCompare
        ...
        response = {
            ...
            'note': f'Analysis complete. Successfully processed {len(results)} out of {total_stocks} assets. Crypto data uses TwelveData with CryptoCompare fallback.',
            **results
        }
...
def home():
    ...
    try:
        ...
        return jsonify({
            'message': 'Enhanced Multi-Asset Analysis API v5.0 - Expanded Dataset',
            'version': '5.0 - TwelveData Crypto + OpenRouter AI + Expanded Dataset',
            'endpoints': {
                '/analyze': 'GET - Get latest analysis (from cache or trigger new)',
                '/analyze/fresh': 'GET - Force fresh analysis (manual refresh)',
                '/progress': 'GET - Get current analysis progress',
...
def health():
    ...
    try:
        ...
        return jsonify({
            ...
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'service': 'Multi-Asset Analysis API with Expanded Dataset - TwelveData Crypto + OpenRouter AI'
        })
    except Exception as e:
        logger.error(f"Error in health endpoint: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Enhanced Multi-Asset Analysis API v5.0 - Expanded Dataset on port {port}")
    logger.info(f"Debug mode: {debug_mode}")
    logger.info(f"Total assets configured: {get_filtered_stocks()['total_count']}")
    logger.info(f"AI Analysis available: {openrouter_client is not None}")
    logger.info("Features: Expanded Dataset (30 US + 15 Nigerian + 20 Crypto) + TwelveData Crypto + OpenRouter AI")
